...
<p id="desc">Since <i>area</i> probably had a large influence in the attribute distance calculations, we are going to normalize the attributes, which will make <i>area</i> only as influencial as the rest.</p>
...
The Dendrogram for the data without the <i>area</i> column is in fact very different from the former two. For example, <strong>Alaska</strong> no longer is by itself from most of the grouping. This is a representation of the other attributes having more pull.
